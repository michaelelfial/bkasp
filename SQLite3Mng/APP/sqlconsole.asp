...
%>    
    ...
    <% If Request.ServerVariables("REQUEST_METHOD") = "POST" And Request("Execute").Count > 0 Then %>
          ...
          <%
            Set sfTimer = Server.CreateObject("newObjects.utilctls.SFMain")
            On Error Resume Next
            timerStart = sf.SystemTicks
            Set r = db.Execute(Request("QUERY"),Request("RFIRST"),Request("RCOUNT"))
            timerEnd = sf.SystemTicks
            ...
            Else
                ...
                If r.Count > 0 Then
                %>
                query(ies) took <%= timerEnd - timerStart %> milliseconds.
                  <table border="0" bgcolor="#000000" cellspacing="1">
                    <%
                        On Error Resume Next
                        For I = 1 To r.Count
                            ...
                            <tr>
                                <% For J = 1 To r(I).Count %>
                                    <% Select Case UCase(r(I).Info(J))
                                        ...
                                        <% Case "REAL" %>
                                            <td VALIGN="TOP" bgcolor="#FFFFFF"><%= su.Sprintf("%NM",r(I)(J)) %></td>
                                        <% Case "TEXT" %>                                                                
                                            <td VALIGN="TOP" bgcolor="#FFFFFF"><%= su.Sprintf("%Na",r(I)(J)) %></td>
                                        <% Case "BLOB" %>                                                                
                ...
                <% End If%>
                Last insert row ID=<%= r.Info %><br/>
                Rows affected=<%= db.Changes %>
