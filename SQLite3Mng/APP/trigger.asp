...
<%
    ...
    Else
        bExecuted = False
        Set r = db.Execute(su.Sprintf("SELECT * FROM sqlite_master WHERE name=%q AND type='trigger';",Object))
    End If
    
    query = Request("QUERY")   
    
    If Request("QUERY").Count = 0 Then
        query = "CREATE TRIGGER <name> BEFORE | AFTER <event> ON <table> WHEN <clause> BEGIN ... END;"
    End If
%>
<html>
