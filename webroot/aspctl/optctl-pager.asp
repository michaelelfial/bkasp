...
Class ButtonPager
    ...
    Public Property Get HttpGetParams
        If ConvertTo(vbBoolean,PreserveInQueryString) Then
            HttpGetParams = Me.Name & "_SP=" & Page & "&" & Me.Name & "_TP=" & TotalPages
        Else
            HttpGetParams = ""
        End If
    End Property
    ...
    Public Sub Render
        If TotalPages > 0 Then
            ...
            If TotalPages > 1 And Page > 1 + nLimit Then
                ' Output GoFirst
                 Response.Write "<input type=""submit"" name=""" & Me.Name & """ value="" 1 """ & _
                                RenderEvents(1) & s & "/>" & FirstDelimiter
            End If
            ...
            For iPage = Page - nLimit To Page + nLimit
                If iPage > 0 And iPage <= TotalPages Then
                    ...
                    If iPage = Page Then
                        Response.Write "<input type=""submit"" name=""" & Me.Name & """ value="" " & iPage & " "" " & IfThenElse(NoDisable,"","DISABLED=""Yes"" ") & _
                                        RenderEvents(iPage) & sCurrent & "/>"
                    Else
                        Response.Write "<input type=""submit"" name=""" & Me.Name & """ value="" " & iPage & " """ & _
                                        RenderEvents(iPage) & s & "/>"
                    End If
            ...
            If TotalPages > 1 And Page < TotalPages - nLimit Then
                ' Output GoFirst
                Response.Write LastDelimiter & "<input type=""submit"" name=""" & Me.Name & """ value="" " & TotalPages & " """ & _
                               RenderEvents(TotalPages) & s & "/>"
            End If
...
Function Create_ButtonPager(controlName)
    Dim ctl
    Set Create_ButtonPager = InitControl(New ButtonPager, True,controlName)
End Function
