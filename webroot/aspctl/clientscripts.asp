...
Const ASPCTL_AsyncEventSink = "ASPCTL_AsyncEventSink"
...
Class CClientScripts
    ...
    ' The standard event sinks are implemented in staticevents-dynamic.js
    Private pAsyncEventSink
    Public Property Let AsyncEventSink(v)
        If Len(v) > 0 Then
            RegisterFile "staticevents-dynamic.js", ASPCTLPath & "staticevents-dynamic.js"
            If Len(pAsyncEventCover) = 0 Then
                pAsyncEventCover = "aspctl_cover"
            End If
        End If
        pAsyncEventSink = v
    End Property
    Public Property Get AsyncEventSink
        AsyncEventSink = pAsyncEventSink
    End Property
    ' The simple standard event sinks require a special div to be rendered in each page ,this supplies the id
    Private pAsyncEventCover
    Public Property Let AsyncEventCover(v)
        pAsyncEventCover = v
    End Property
    Public Property Get AsyncEventCover
        AsyncEventCover = pAsyncEventCover
    End Property
    ' Lazy? Call this in your master pages in the body part to render a simple cover div
    Public Sub RenderStandardAsyncCover
        %>
        <div id="aspctl_cover" style="background-image: url(<%= VirtPath("/aspctl/img/bkgnd-black-25.png") %>);text-align:center;position:absolute;display:none;z-index:1000;" 
            onclick="ccPriorityLoader.cancelCurrent();"
            title="<%= TR("Click to cancel") %>">
            <table style="width:100%;height:100%;"><tr><td style="vertical-align:middle;text-align:center;">
                <img src="<%= VirtPath("/aspctl/img/working.gif") %>" alt="" style="margin:auto;"/>
            </td></tr></table>
        </div>
        <%
    End Sub
    Public Sub RenderAsyncCover(backImg,progressImg)
        %>
        <div id="aspctl_cover" style="background-image: url(<%= VirtPath(backImg) %>);text-align:center;position:absolute;display:none;" 
            onclick="ccPriorityLoader.cancelCurrent();"
            title="<%= TR("Click to cancel") %>">
            <table style="width:100%;height:100%;"><tr><td style="vertical-align:middle;text-align:center;">
                <img src="<%= VirtPath(progressImg) %>" alt="" style="margin:auto;"/>
            </td></tr></table>
        </div>
        <%
    End Sub
            ...
            For I = 1 To eh.Count
                If eh.Key(I) <> "" And Left(eh.Key(I),1) <> "$" Then
                    ...
                    if Not IsEmpty(plusEvent) And eh.Key(I) = plusEvent Then
                        ...
                        Else
                            strexec = strexec & plusHandler & "(ccStaticEvent.Event" & plusParams & ");"
                        End If
                    End If
    ...
    Function GetSingleEventHandler(funcName,evntName,params)
        GetSingleEventHandler = funcName & "(ccStaticEvent.NewEvent(this,(arguments.length>0?arguments[0]:null),'" & evntName & "')" & IfThenElse(Len(params) > 0,"," & params,"") & ");"
    End Function
    ...
    Sub SetAsyncPostBackFocus(ctl, eventName, uCtl, ctlFocus, ctlName, ctlVal)
        ...
        Else
            If ConvertTo(vbString,ctlName) <> "" Then
                RegisterEventHandler ctl, eventName, "StaticAsyncPostBack", "'" & CurrentFormName & "','" & uCtl.Name & "','" & ctlFocus.ClientId & "','" & ctlName & "','" & ctlVal & "',null,'" & uCtl.ClientId & "'", "ASPCTL_ASYNCPOSTBACK"
            Else
                RegisterEventHandler ctl, eventName, "StaticAsyncPostBack", "'" & CurrentFormName & "','" & uCtl.Name & "','" & ctlFocus.ClientId & "',null,null,null,'" & uCtl.ClientId & "'", "ASPCTL_ASYNCPOSTBACK"
            End If
        End If
    End Sub
    ...
    Function RenderAsyncAndControlEventHandlers(ctl,eventName, uCtl, ctlName, ctlVal)
        If ASPCTL_AsyncRequestsEnabled Then
            RenderAsyncAndControlEventHandlers = RenderControlEventHandlersEx(ctl,eventName,"StaticAsyncPostBack","'" & CurrentFormName & "','" & uCtl.Name & "','" & uCtl.ClientId & "','" & ctlName & "','" & ctlVal & "',null,'" & uCtl.ClientId & "'")
        Else
    ...
    Function RenderAsyncAndButtonEventHandlers(ctl,eventName, uCtl, ctlName, ctlVal)
        If ASPCTL_AsyncRequestsEnabled Then
            RenderAsyncAndButtonEventHandlers = RenderControlEventHandlersEx(ctl,eventName,"StaticButtonAsyncPostBack","'" & CurrentFormName & "','" & uCtl.Name & "','" & uCtl.ClientId & "','" & ctlName & "','" & ctlVal & "',null,'" & uCtl.ClientId & "'")
        Else
    ...
    Public Sub Render
        ...
        If Len(pAsyncEventSink) > 0 Then
            Response.Write "var " & ASPCTL_AsyncEventSink & ";" & vbCrLf
        End If
        ...
        Response.Write "<script type=""text/javascript"">" & vbCrLf
            If Initializers.Count > 0 Then
                Response.Write "function " & ASPCTL_PageLoadUnloadProcPrefix & "OnLoad() {" & vbCrLf
                    ...
                    Next
                    If Len(pAsyncEventSink) > 0 Then
                        Response.Write ASPCTL_AsyncEventSink & "= new " & pAsyncEventSink & "('" & pAsyncEventCover & "');" & vbCrLf
                    End If
