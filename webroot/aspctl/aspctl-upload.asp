...
Class CASPCTLFile
    ...
    Public Property Get TextData
        Content.Pos = 0
        TextData = Content.ReadText(-2)
    End Property
	Public Property Get BOMTextData
		BOMTextData = ReadBOMAndConfigure(Content)
	End Property
    Public Property Get Stream
        Set Stream = Content
    End Property
End Class
Function ReadBOMAndConfigure(strm)
	Dim bd, v
	ReadBOMAndConfigure = ""
	...
	v = strm.ReadBin(5)
	If IsEmpty(v) Then
		strm.Pos = 0 ' No config possible - go to default mode (UTF-8)
		strm.unicodeText = false
		strm.codePage = 65001
		ReadBOMAndConfigure = strm.ReadText(-2)
		Exit Function
	End If
	bd.Value = v
	' Check for UTF-8 BOM
	If bd.Size >= 3 Then
		If bd.Byte(0) = &HEF And bd.Byte(1) = &HBB And bd.Byte(2) = &HBF Then
			strm.unicodeText = false
			strm.codePage = 65001
			strm.Pos = 3
			ReadBOMAndConfigure = strm.ReadText(-2)
	...
	End If
	If bd.Size >= 2 Then
		If bd.Byte(0) = &HFE And bd.Byte(1) = &HFF Then
			strm.unicodeText = true
			strm.Pos = 2
			bd.Value = strm.ReadBin(strm.Size - 2)
			bd.UnicodeByteOrder = 2
			ReadBOMAndConfigure = bd.UnicodeString(0)
			Exit Function
		End If
		If bd.Byte(0) = &HFF And bd.Byte(1) = &HFE Then
			strm.unicodeText = true
			strm.Pos = 2
			ReadBOMAndConfigure = strm.ReadText(-2)
			Exit Function
...
Class CASPCTLUpload
    ...
    Private Sub Class_Initialize()
        ...
        Dim nRead, nToRead, nTotal
        nRead = 0
        nTotal = Request.TotalBytes
        Do
            nToRead = 65536
            If nTotal - nRead < nToRead Then nToRead = nTotal - nRead
            postStream.WriteBin Request.BinaryRead(nToRead)
            nRead = nRead + nToRead
        Loop While nRead < nTotal
        postStream.Pos = 0
    ...
    Private Sub BuildUploadRequest()
        ...
        Do
            ...
            If field("IsFile") Then
                ...
                If field("ContentLength") > 0 Then 
                    postStream.CopyTo o.Stream, field("ContentLength")
                End If
                o.Stream.Pos = 0
                postStream.Pos = temp
...
Function ParseJSONPost()
        ...
        Loop While nRead < nTotal
        postStream.Pos = 0
		postStream.codePage = 65001
		Dim str
		str = postStream.ReadText(-2)
		Set ParseJSONPost = JSON.Parse(str)
End Function

Dim ASPGET, ASPPOST, ASPFILES, ASPVARS, ASPJSON
Dim ASP_CONTENT_TYPE
ASP_CONTENT_TYPE = CStr(Request.ServerVariables("CONTENT_TYPE"))
n = Instr(ASP_CONTENT_TYPE,";")
If n > 0 Then ASP_CONTENT_TYPE = Trim(Left(ASP_CONTENT_TYPE,n - 1))
Dim JSON
...
ElseIf LCase(ASP_CONTENT_TYPE) = "application/json" Or 	LCase(ASP_CONTENT_TYPE) = "text/x-json" Then
	ASPCTL_FileUpload = False
	Set ASPPOST = Server.CreateObject("newObjects.utilctls.VarDictionary")
	Set ASPPOST.Missing = ASPPOST.CreateNew
